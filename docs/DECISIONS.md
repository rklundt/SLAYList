...
**Validation posture (Sprint 1.6 acceptance, load-bearing):** "the Bicep faithfully re-creates the live dev RG" is the claim that must be verified, NOT assumed. Per the verify-don't-assume discipline used elsewhere in this file (**D18**, **D27**, **D34**, **D36**, **D37**), Sprint 1.6 validates in **two complementary passes**:

1. **Forward pass — deploy to a throwaway RG.** Deploy `main.bicep` to `rg-music-slaylist-validate-use2` (env=validate), then diff `az resource list` + spot-check key properties against the live dev RG. Proves the templates *create* a faithful copy from nothing (the prod/Sprint-9.1 path).
2. **Reverse pass — `az deployment group what-if` against the live dev RG.** ARM's own dry-run engine computes the exact delta a real deploy *would* apply to dev, changing nothing. This is the authoritative "is it safe to run on the real environment" check, and it catches property-level drift the resource-list diff can't see (the throwaway RG has no pre-existing state to diff against). The two passes found five real Phase-A capture gaps that a single pass would have missed — most importantly a blob soft-delete retention hardcoded to 7 days when dev runs 30 (running the Bicep as-authored would have *shrunk the kids'-songs recovery window*, a data-safety regression). Also corrected: Container App `maxReplicas` (3→1), placeholder image, removal of a placeholder env var dev doesn't carry, and budget-notification key naming.

If a real-RG resource fails to materialize from the Bicep, the Bicep is fixed before merge; if it's a property that genuinely cannot be captured (e.g. a portal-only or computed field), that gap is recorded as a known-limitation note rather than silently accepted.

**Known what-if interpretation notes (so a future reader isn't alarmed by a noisy dry-run):**
- **Computed/read-only/default-applied fields show as spurious deletes.** `stableInboundIP`, `trafficSplitting`, `runningStatus`, `maxInactiveRevisions`, container `defaultEncryptionScope`, blob `allowPermanentDelete`, CAE `peerAuthentication`/mtls, and classic queue `logging` all appear as `Delete` in what-if because the template doesn't set them. ARM does NOT actually remove them on apply — they're platform defaults or read-only status.
- **Array properties and `reference()` expressions show as unevaluated.** Diagnostic-setting `logs`/`metrics`, the Container App `containers` array, and the CAE `customerId` (a `reference()` to the same LAW) show as `Array None→None` or unresolved-expression diffs because what-if can't evaluate them. These were verified equal to dev by direct query.
- **`deploymentAuthPolicy` on the SWA is a real GA ARM property that Bicep 0.43.8's type set does not yet list** (emits BCP037). It is pinned to `DeploymentToken` (matching dev, preserving the D37 break-glass path) with a `#disable-next-line BCP037` directive; the value passes through to ARM correctly (verified on the deployed resource).
- **Role-assignment GUIDs differ between portal-created and Bicep-created assignments.** The Bicep names role assignments deterministically via `guid(scope, principal, roleId)`; dev's existing assignments (created in Sprint 1.4 via portal) carry random GUIDs. A what-if against dev therefore shows the two Bicep assignments as `Create`. **On a *fresh* deploy (prod, Sprint 9.1) this is exactly right.** On a *reconcile of dev*, those creates would hit `RoleAssignmentExists` (409) because Azure rejects a duplicate principal+role+scope even under a new assignment name — so reconciling dev requires deleting the two portal-created assignments first (brief RBAC gap on a scale-to-zero placeholder, acceptable), or leaving dev's RBAC as-is and accepting the template/live divergence on those two resources only. This caveat is dev-reconcile-specific and does NOT affect the prod build.
