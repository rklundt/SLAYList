...
- **The Bicep in `infra/bicep/` is the source of truth for Azure infrastructure (D39). Keep it in sync — every infra change updates the Bicep in the same sprint.** Since Sprint 1.6, the Epic 1 landing zone is captured as Bicep modules (`main.bicep` orchestrating `naming`/`observability`/`swa`/`storage`/`containerapp`/`eventgrid`). Any sprint that adds, removes, or reconfigures an Azure resource — a new RBAC role, a queue, a diagnostic setting, a scale setting, a container, a lifecycle rule — **must edit the corresponding Bicep module as part of that sprint's work, not "later."** Portal-only changes are drift: they make the next `what-if` lie and turn prod (Sprint 9.1, deployed *from* this Bicep with `env=prod`) into a silent mismatch with dev. The discipline: (1) make the change, (2) reflect it in the Bicep, (3) `bicep build` clean, (4) for changes against an already-deployed env, `az deployment group what-if` to confirm only the intended delta — `infra/bicep/drift-check.ps1` (added Sprint 1.7) wraps this as a noise-filtered CLEAN/DRIFT check and is the quickest way to confirm live still matches the templates (the live dev RG was reconciled to `managed-by=bicep` at Sprint 1.7). Known deferrals are tracked in-Bicep as comments (e.g. the Event Grid subscription + its queue RBAC are Epic 7, noted in `eventgrid.bicep`/`containerapp.bicep`). `/wrap-sprint` DevOps verifies the Bicep matches what the sprint actually built. The secrets posture is fixed: **non-sensitive outputs only** (resource IDs, MI principal IDs, hostnames) — storage keys and SWA tokens are fetched post-deploy via `az`, never emitted as Bicep outputs (D17/D32/D39). **One deliberate exception (D41):** the GitHub-Actions OIDC deploy identity (Entra app registration + federated credential + its RBAC) is a Microsoft Graph object, not an ARM resource, and is the bootstrap credential the Bicep itself runs under — so it lives in `scripts/bootstrap-deploy-identity.ps1` (an idempotent, env-parameterized seed script), NOT in `infra/bicep/`. Same logic applies to the Epic 3 user-auth Entra app. Graph/Entra identity objects = bootstrap script; Azure ARM resources = Bicep.
