...
- **Vitest is wired up (D23)** at the root and per-workspace: `npm test` runs Vitest across `/shared`, `/frontend`, `/api`, and `/transcoder` workspaces; a placeholder `*.test.ts` in `/shared` proves a test can run and pass. CI integration lands in Epic 2 (soft mode until Epic 4.1 hardens it).
...
- **Re-attempt CodeQL default-setup (D31, deferred from Sprint 0.1).** Once TypeScript code is present in `/shared` (or any workspace), enable in the GitHub UI (Security → Code scanning → Set up → Default) OR via `gh api -X PUT repos/rklundt/SLAYList/code-scanning/default-setup --input <{"state":"configured","query_suite":"default"}>`. Verify configured: `gh api repos/rklundt/SLAYList/code-scanning/default-setup --jq '.state'` returns `"configured"`. If still 404 or fails with a different error, escalate as a Sprint-0.2 finding rather than silently dropping D31's CodeQL requirement.
