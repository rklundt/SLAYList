...
- ⬜ 1.6 IaC capture (Bicep) — capture all Sprint 1.1–1.5 resources as Bicep modules so dev is rebuildable from code, prod (Sprint 9.1) is a parameter-substitution deploy, and future infra changes go through reviewable PRs. Validated **two ways**: deploy-to-throwaway-RG + `az resource list` comparison, AND `az deployment group what-if` against live dev (which caught 6 Phase-A capture gaps incl. a blob soft-delete retention that would have shrunk the recovery window, and a wrong RBAC role). Secrets stay OUT (deployment outputs, not inputs). D39 records the choice + two-pass posture + what-if interpretation notes.
- ⬜ 1.7 Reconcile live dev RG to Bicep (**optional, does NOT block Epic 1 closure or Epic 2**) — apply the Bicep to the live `rg-music-slaylist-dev-use2` so dev carries `managed-by=bicep` and future dev changes flow through PRs. Handles the documented role-assignment-409 caveat (delete dev's portal-created assignments first). Fresh prod (Sprint 9.1) does NOT depend on this.
- **Exit:** dev resources exist in portal AND are captured as IaC; manual-portal scope ✅ at Sprint 1.5; IaC capture closes at Sprint 1.6. (Sprint 1.7 dev-reconcile is optional tidiness, tracked separately.)
