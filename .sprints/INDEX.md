...
**Status: ✅ DONE.** Every dev Azure resource (RG, LAW + App Insights + budget, SWA, storage with containers/table/queues/diagnostics, Container App env + app + MI + RBAC, Event Grid system topic) exists in the portal AND is captured as env-neutral Bicep under `infra/bicep/` (D39), validated two ways (throwaway-RG deploy + `what-if` against live dev) at Sprint 1.6. Exit criterion met at 1.6. Sprint 1.7 (reconcile the live dev RG to `managed-by=bicep`) is optional tidiness and explicitly does NOT gate Epic 1 closure or Epic 2.
