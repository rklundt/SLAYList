...
4. **Produce a plan** for the human containing:
   ...
   - **For `[Human, guided]` (or `[Both]`) sprints:** explicitly split the plan into **Phase A** (Claude Code's pre-work — guides, scaffolding, code that must exist before the human acts) and **Phase B** (the human's manual execution — portal clicks, account creation, anything Claude Code cannot do). For each acceptance criterion in the sprint file, tag it `[Phase A]` or `[Phase B]`. Acceptance criteria that span both phases are split into the smallest atomic bullets that fit one phase each. `/wrap-sprint` cannot proceed until both phases are complete per the STOP CHECK in `wrap-sprint.md`; the explicit split makes that check tractable and prevents the silent-Phase-B-pending failure mode.
